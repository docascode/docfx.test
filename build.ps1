...
$expected = Get-ChildItem -Path "$expectedPath" | foreach {Get-FileHash -Path $_.FullName}
$actual = Get-ChildItem -Path "$actualPath" | foreach {Get-FileHash -Path $_.FullName}
